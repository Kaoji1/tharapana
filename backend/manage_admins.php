...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_admin') {
        ...
        if ($password !== $confirm_password) {
            ...
            if (mysqli_stmt_num_rows($stmt_check) > 0) {
                ...
                // ใช้ 'password_hash' (โค้ดเดิมของคุณ)
                ...
                if (mysqli_stmt_execute($stmt_insert)) {
                    // ⭐️ 3.1 LOGGING: บันทึก Log การ "เพิ่ม"
                    $new_admin_id = mysqli_insert_id($conn); // ดึง ID ของแอดมินที่เพิ่งสร้าง
                    $message = 'เพิ่มข้อมูลผู้ดูแลระบบสำเร็จ!';
                    $message_type = 'success';
                    
                    $log_details = "สร้าง Admin ใหม่: ID: $new_admin_id, ชื่อ: '$first_name $last_name', อีเมล: '$email'";
                    create_log($conn, $current_admin_id, 'CREATE_ADMIN', $new_admin_id, $log_details);
    ...
    if ($action === 'update_admin') {
        ...
        // ⭐️ 3.2 LOGGING: 1. ดึงข้อมูลเก่า "ก่อน" แก้ไข
        $old_data = null;
        $sql_old = "SELECT first_name, last_name, email, phone FROM users WHERE user_id = ? AND role = 'admin'";
        ...
        $sql_update = "UPDATE users SET first_name = ?, last_name = ?, email = ?, phone = ?, updated_at = NOW() WHERE user_id = ? AND role = 'admin'";
        $stmt_update = mysqli_prepare($conn, $sql_update);
        mysqli_stmt_bind_param($stmt_update, "ssssi", $_POST['first_name'], $_POST['last_name'], $_POST['email'], $_POST['phone'], $target_user_id);
        
        if (mysqli_stmt_execute($stmt_update)) {
            $message = 'แก้ไขข้อมูลผู้ดูแลระบบสำเร็จ!';
            $message_type = 'success';
            
            // ⭐️ 3.2 LOGGING: 2. บันทึก Log "หลัง" แก้ไขสำเร็จ
            if ($old_data) {
                $details_arr = [];
                if ($old_data['first_name'] != $_POST['first_name']) $details_arr[] = "ชื่อ: '{$old_data['first_name']}' -> '{$_POST['first_name']}'";
                if ($old_data['last_name'] != $_POST['last_name']) $details_arr[] = "นามสกุล: '{$old_data['last_name']}' -> '{$_POST['last_name']}'";
                if ($old_data['email'] != $_POST['email']) $details_arr[] = "อีเมล: '{$old_data['email']}' -> '{$_POST['email']}'";
                if ($old_data['phone'] != $_POST['phone']) $details_arr[] = "เบอร์โทร: '{$old_data['phone']}' -> '{$_POST['phone']}'";
                
                $details = "แก้ไข Admin ID: $target_user_id. " . (empty($details_arr) ? " (ไม่มีการเปลี่ยนแปลงข้อมูล)" : implode(", ", $details_arr));
                create_log($conn, $current_admin_id, 'UPDATE_ADMIN', $target_user_id, $details);
            }
    ...
    if ($action === 'delete_admin') {
        $admin_id_to_delete = (int)$_POST['user_id'];
        ...
        if ($admin_id_to_delete === $current_admin_id) {
            ...
            // ⭐️ 3.3 LOGGING: 1. ดึงข้อมูล "ก่อน" ลบ
            $user_info = "ID: $admin_id_to_delete"; // ค่าเริ่มต้น
            $sql_old_del = "SELECT first_name, last_name, email FROM users WHERE user_id = ? AND role = 'admin'";
            $stmt_old_del = mysqli_prepare($conn, $sql_old_del);
            mysqli_stmt_bind_param($stmt_old_del, "i", $admin_id_to_delete);
            if (mysqli_stmt_execute($stmt_old_del)) {
                ...
                if ($old_data_del = mysqli_fetch_assoc($result_old_del)) {
                    $user_info = "ID: $admin_id_to_delete, ชื่อ: '{$old_data_del['first_name']} {$old_data_del['last_name']}', อีเมล: '{$old_data_del['email']}'";
                }
            }
            mysqli_stmt_close($stmt_old_del);
            // --- จบส่วนดึงข้อมูล ---

            // ทำการ DELETE (โค้ดเดิม)
            ...
            if (mysqli_stmt_execute($stmt_delete)) {
                $message = 'ลบข้อมูลผู้ดูแลระบบสำเร็จ!';
                $message_type = 'success';
                
                // ⭐️ 3.3 LOGGING: 2. บันทึก Log "หลัง" ลบสำเร็จ
                $details = "ลบ Admin: " . $user_info;
                create_log($conn, $current_admin_id, 'DELETE_ADMIN', $admin_id_to_delete, $details);
...
if ($total_pages < 1) $total_pages = 1; // ป้องกันหน้าเป็น 0
if ($current_page > $total_pages) $current_page = $total_pages; // ถ้าหน้าเกิน ให้ไปหน้าสุดท้าย

mysqli_stmt_close($stmt_count);

$admins = [];
$sql_select = "SELECT user_id, first_name, last_name, email, phone, created_at FROM users $where_clause ORDER BY created_at DESC LIMIT ? OFFSET ?";
...
<div class="card">
    <div class="card-header d-flex flex-wrap justify-content-between align-items-center gap-2">
        <h5 class="card-title mb-0"><i class="bi bi-shield-lock-fill me-2"></i>จัดการผู้ดูแลระบบ (Admins)</h5>
        <div class="d-flex gap-2">
            <form method="GET" action="admin.php" class="d-flex" role="search">
                <input type="hidden" name="page" value="manage_admins">
                <input class="form-control form-control-sm me-2" type="search" name="search" placeholder="ค้นหา..." value="<?= htmlspecialchars($search_query) ?>" aria-label="Search">
            ...
            <button class="btn btn-sm btn-success" data-bs-toggle="modal" data-bs-target="#addAdminModal">
                <i class="bi bi-plus-circle-fill"></i> เพิ่มผู้ดูแลระบบ
            </button>
        </div>
    </div>
    <div class="card-body">
                    ...
                    <?php if (empty($admins)): ?>
                        <tr>
                            <td colspan="6" class="text-center text-muted py-3">
                                <?= !empty($search_query) ? "ไม่พบข้อมูลที่ตรงกับ '" . htmlspecialchars($search_query) . "'" : "ไม่พบข้อมูลผู้ดูแลระบบ" ?>
                            </td>
                        </tr>
                    <?php else: ?>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <button class="btn btn-sm btn-primary edit-btn"
                                        ...
                                        data-fname="<?= htmlspecialchars($admin['first_name'], ENT_QUOTES) ?>"
                                        data-lname="<?= htmlspecialchars($admin['last_name'], ENT_QUOTES) ?>"
                                        data-email="<?= htmlspecialchars($admin['email'], ENT_QUOTES) ?>"
                                        data-phone="<?= htmlspecialchars($admin['phone'], ENT_QUOTES) ?>">
                                    <i class="bi bi-pencil-square"></i> แก้ไข
                                </button>
                                <?php if ($admin['user_id'] != $current_admin_id): // โค้ดเดิม: ป้องกันการลบตัวเอง ?>
                                <button class="btn btn-sm btn-danger delete-btn" 
                                        data-id="<?= $admin['user_id'] ?>" 
                                        data-name="<?= htmlspecialchars($admin['first_name'] . ' ' . $admin['last_name'], ENT_QUOTES) ?>">
                                    <i class="bi bi-trash3-fill"></i> ลบ
                                </button>
                                <?php else: ?>
                                    <button class="btn btn-sm btn-secondary" disabled>
                                        <i class="bi bi-person-badge"></i> (คุณ)
                                    </button>
                                <?php endif; ?>
                            </td>
                        </tr>
        ...
        <nav aria-label="Page navigation">
            <ul class="pagination justify-content-center mt-3">
                <li class="page-item <?= ($current_page <= 1) ? 'disabled' : '' ?>">
                    <a class="page-link" href="?page=manage_admins&search=<?= urlencode($search_query) ?>&p=<?= $current_page - 1 ?>">Previous</a>
                </li>
                <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                <li class="page-item <?= ($i == $current_page) ? 'active' : '' ?>">
                    <a class="page-link" href="?page=manage_admins&search=<?= urlencode($search_query) ?>&p=<?= $i ?>"><?= $i ?></a>
                </li>
                <?php endfor; ?>
                <li class="page-item <?= ($current_page >= $total_pages) ? 'disabled' : '' ?>">
                    <a class="page-link" href="?page=manage_admins&search=<?= urlencode($search_query) ?>&p=<?= $current_page + 1 ?>">Next</a>
                </li>
            </ul>
        </nav>
        <?php endif; ?>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="addAdminModalLabel"><i class="bi bi-person-plus-fill me-2"></i>เพิ่มผู้ดูแลระบบใหม่</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form id="addAdminForm" method="POST" action="admin.php?page=manage_admins">
        ...
        <div class="modal-footer">
          ...
          <button type="submit" class="btn btn-success">เพิ่มผู้ดูแลระบบ</button>
        </div>
      </form>
    </div>
  </div>
...
<div class="modal fade" id="editAdminModal" tabindex="-1" aria-labelledby="editAdminModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="editAdminModalLabel"><i class="bi bi-pencil-fill me-2"></i>แก้ไขข้อมูลผู้ดูแลระบบ</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form id="editAdminForm" method="POST" action="admin.php?page=manage_admins&search=<?= urlencode($search_query) ?>&p=<?= $current_page ?>">
        <div class="modal-body">
...
<form id="deleteAdminForm" method="POST" action="admin.php?page=manage_admins&search=<?= urlencode($search_query) ?>&p=<?= $current_page ?>" style="display: none;">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (editAdminModal) {
        editAdminModal.addEventListener('show.bs.modal', function(event) {
            ...
            const modalForm = editAdminModal.querySelector('form');
    ...
    // --- ⭐️ 2. อัปเกรด: ยืนยันการ "เพิ่ม" (ใช้ SweetAlert2) ---
    ...
    if (addAdminForm) {
        addAdminForm.addEventListener('submit', function(event) {
            ...
            Swal.fire({
                ...
                text: "คุณต้องการเพิ่มผู้ดูแลระบบคนนี้ใช่หรือไม่?",
                icon: 'question',
    ...
    // --- ⭐️ 3. อัปเกรด: ยืนยันการ "แก้ไข" (ใช้ SweetAlert2) ---
    ...
    // --- ⭐️ 4. อัปเกรด: ยืนยันการ "ลบ" (ใช้ SweetAlert2) ---
    ...
    deleteButtons.forEach(button => {
        button.addEventListener('click', function(event) {
            ...
            Swal.fire({
                ...
                html: `คุณต้องการลบบัญชี Admin ของ <strong>'${userName}'</strong> อย่างถาวรใช่หรือไม่?<br><strong class='text-danger'>การกระทำนี้ไม่สามารถย้อนกลับได้!</strong>`,
                icon: 'warning',
                showCancelButton: true,
